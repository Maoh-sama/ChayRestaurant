...
namespace Cuahangchay.Services
{
    public class VnPayLibrary
    {
        ...
        {
            return _requestData.TryGetValue(key, out string retValue) ? retValue : string.Empty;
        }
        ...
        {
            StringBuilder data = new StringBuilder();
            if (_requestData.ContainsKey("vnp_SecureHashType")) _requestData.Remove("vnp_SecureHashType");
            if (_requestData.ContainsKey("vnp_SecureHash")) _requestData.Remove("vnp_SecureHash");
            foreach (KeyValuePair<string, string> kv in _requestData)
            {
                if (data.Length > 0) data.Append('&');
                data.Append(kv.Key + "=" + WebUtility.UrlEncode(kv.Value));
